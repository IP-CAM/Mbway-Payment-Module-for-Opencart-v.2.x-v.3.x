...
class ControllerExtensionPaymentIfthenpayMbway extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link('extension/payment/ifthenpaymbway', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/ifthenpaymbway', 'token=' . $this->session->data['token'], 'SSL')
        );
        ...
        $data['action'] = $this->url->link('extension/payment/ifthenpaymbway', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['ifthenpaymbway_url'] = ($this->config->get('config_secure') ? HTTP_CATALOG : HTTPS_CATALOG) . "index.php?route=extension/payment/ifthenpaymbway/callback&chave=[CHAVE_ANTI_PHISHING]&referencia=[REFERENCIA]&idpedido=[ID_TRANSACAO]&valor=[VALOR]&estado=[ESTADO]";

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/payment/ifthenpaymbway.tpl', $data));
    }
